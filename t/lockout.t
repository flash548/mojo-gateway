...
subtest 'Check locks clears bad attempts after successful login' => sub {
  ...
  $t->post_ok('/auth/login', form => {username => 'admin@test.com', password => 'testpass'})
    ->status_is(Constants::HTTP_OK)->content_unlike(qr/ Login failed: User or password incorrect!/, 'Login');
...
subtest 'Test that account can be administratively locked' => sub {
  ...
  # verify no access
  $t->get_ok('/admin')->status_is(Constants::HTTP_OK)->content_like(qr/account is locked/i);

  # unlock the account manually
  $t->app->db_conn->db->update('users', {locked => 0}, {email => 'admin@test.com'});

  # verify access
  $t->get_ok('/admin')->status_is(Constants::HTTP_OK)->content_unlike(qr/account is locked/i);
...
subtest 'Test that bad attempts are cleared out on an admin-resetting action' => sub {
  ...
  $t->post_ok('/auth/login', form => {username => 'dude2@test.com', password => 'testpass'})
    ->status_is(Constants::HTTP_OK)->content_like(qr/ Login failed: User or password incorrect!/, 'Login');
  ...
  $t->post_ok('/auth/login', form => {username => 'dude2@test.com', password => 'testpass'})
    ->status_is(Constants::HTTP_OK)->content_like(qr/account is locked/i, 'Login');
  ...
  $t->put_ok('/admin/users', json => $acct)->status_is(Constants::HTTP_OK);

...
my $t = Test::Mojo->new(
    'Gateway',
    {
        ...
        routes     => {
            ...
            '/everyone' =>
              { requires_login => 0, uri => "http://localhost:8080/everyone" },
            '/api' => {
        ...
        default_route =>
          { uri => 'https://localhost:8080/frontend', requires_login => 1 },
...
subtest 'Test User Login/Logout/Admin operations' => sub {
    ...
    $t->get_ok('/')->status_is(Constants::HTTP_OK)
    ...
    $t->post_ok( '/auth/login',
        form => { username => 'admin@test.com', password => 'testpass' } )
      ->status_is(Constants::HTTP_OK)
      ->content_unlike( qr/login/i,
        'Make sure we are not at the login page anymore' );

    # since we're an admin we should get the admin page at /admin
    $t->get_ok('/admin')->status_is(Constants::HTTP_OK)
...
# log back in - should land at the admin page since thats what we last tried to get to
    $t->post_ok( '/auth/login',
        form => { username => 'admin@test.com', password => 'testpass' } )
      ->status_is(Constants::HTTP_OK)
      ->content_like( qr/Admin/i,
        'Log Back In Successfully - Land at Admin Page' );
...
# return should be the new user record with expected fields - WITHOUT the password field hash
    ...
    )->status_is(Constants::HTTP_CREATED)->json_has('/email')
      ->json_has('/user_id')->json_has('/is_admin')
      ->json_has('/reset_password')->json_has('/last_reset')
      ->json_has('/last_login')->json_hasnt('/password');

    # try to add them again and get a 409 (already exists)
    $t->post_ok( '/admin/users',
        json =>
          { email => 'test@test.com', password => 'password', is_admin => 0, } )
...
# try to add them again and get a 409 (already exists)
    ...
    $t->post_ok( '/auth/login',
        form => { username => 'test@test.com', password => 'password' } )
      ->status_is(Constants::HTTP_OK)
      ->element_exists('[name=current-password]')
      ->element_exists('[name=new-password]')
      ->element_exists('[name=retyped-new-password]')
      ->content_like( qr/Change/i,
        'Log Back In - Get Force password change screen' );
    ...
    $t->post_ok( '/auth/password/change',
      ...
      ->status_is(Constants::HTTP_OK)
      ->content_like( qr/Password Change Failed: Required fields not/,
        'Missing re-typed field' );

    # change password - provide bad existing one - fails with error
    $t->post_ok(
        '/auth/password/change',
        form => {
            'current-password'     => 'blah',
            'new-password'         => 'blah2',
            'retyped-new-password' => 'blah2'
        }
    )->status_is(Constants::HTTP_OK)
      ->content_like( qr/Password Change Failed: Existing/,
        'Existing Password was wrong' );
    ...
    $t->post_ok(
        ...
        form => {
            ...
            'retyped-new-password' => 'blah3'
        }
    )->status_is(Constants::HTTP_OK)->content_like(
qr/Password Change Failed: New Password cannot equal the existing password/,
...
qr/Password Change Failed: New Password does not equal Retyped New Password/,
...
qr/Password Change Failed: New Password does not meet complexity requirements/,
...
qr/Password Change Failed: Passwords cannot exceed 255 chars/,
    ...
    $t->post_ok(
        ...
        form => {
            'current-password'     => 'password',
            'new-password'         => 'blah234!',
            'retyped-new-password' => 'blah234!'
        }
    )->status_is(Constants::HTTP_OK)
      ->content_like( qr/Whoa/,
        'Password Change OK - Re-directs to root route' );

    # check that non-admin's can't access the admin interface
    $t->get_ok('/admin')
      ->status_is( Constants::HTTP_FORBIDDEN,
        'Non-Admins cant access the Admin interface' );

    # check that non-admin's can't access the admin API either
    $t->get_ok('/admin/users')
      ->status_is( Constants::HTTP_FORBIDDEN,
        'Non-admins cant access Admin API' );
    ...
    $t->put_ok( '/admin/users',
        json =>
          { email => 'test@test.com', user_id => '222222', callsign => 'japh' }
    )->status_is(Constants::HTTP_OK)->json_is( '/email' => 'test@test.com' )
      ->json_is( '/user_id'        => 222222 )->json_is( '/is_admin' => 0 )
      ->json_is( '/reset_password' => 0 )
      ->json_unlike( '/last_reset' => qr/2022-01-01T00:00:00Z/ )
      ->json_hasnt('/callsign')->json_hasnt('/password');

    # expire it manually
    $t->app->db_conn->db->update(
        "users",
        { last_reset => "2022-01-01T00:00:00Z" },
        { email      => "test\@test.com" }
    );
    ...
    $t->post_ok( '/auth/login',
        form => { username => 'test@test.com', password => 'blah234!' } )
      ->status_is(Constants::HTTP_OK)
      ->element_exists('[name=current-password]')
      ->element_exists('[name=new-password]')
      ->element_exists('[name=retyped-new-password]')
      ->content_like( qr/Change/i,
        'Log Back In - Get Force password change screen again' );
    ...
    $t->delete_ok('/admin/users')
      ->status_is( Constants::HTTP_BAD_REQUEST, 'Bad request' );
    $t->delete_ok('/admin/users?email=test2@test.com')
      ->status_is( Constants::HTTP_NOT_FOUND, 'Bogus user delete' );
    $t->delete_ok('/admin/users?email=test@test.com')
      ->status_is( 204, 'Successfully deleted user' );
...
subtest 'Test user account updates cannot modify read-only fields' => sub {
    ...
    $t->get_ok('/admin/users?email=admin@test.com')
      ->status_is(Constants::HTTP_OK)->tap(
        sub ($t) {
...
# post an update to self as admin to try to changed the last_reset and last_login fields
    $t->put_ok( '/admin/users',
        json =>
          { email => 'admin@test.com', last_reset => '2022-08-01T12:00:00Z' } )
      ->status_is(Constants::HTTP_OK);
...
subtest
  'Test that routes not requiring authentication work without logging in' =>
  sub {
...
subtest 'Test that other users cant change other users passwords' => sub {
    ...
    $t->post_ok( '/auth/login',
        form => { username => 'dude2@test.com', password => 'dude2!' } )
      ->status_is(Constants::HTTP_OK)
      ->content_like( qr/Whoa/, 'Non-admin logged in' );

    # test gets 403 when doing it via API
    $t->put_ok( '/admin/users',
        json => { email => 'admin@test.com', password => 'hacked' } )
      ->status_is(Constants::HTTP_FORBIDDEN);
};

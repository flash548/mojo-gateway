...
$t->get_ok('/')->status_is(200)->content_like(qr/login/i, 'Test Login screen landing 1');
$t->get_ok('/api')->status_is(200)->content_like(qr/login/i, 'Test Login screen landing 2');

# test we get the login page form elements
$t->get_ok('/api')->status_is(200)->content_like(qr/login/i, 'Test Login screen landing 3')
...
$t->get_ok('/admin')->status_is(200)->content_like(qr/Admin/, 'Make sure we are at the Admin page');

# test we get the "Whoa!!" error page since we have no server to proxy to
$t->get_ok('/api')->status_is(200)->content_like(qr/Whoa/i, 'Test Routing Failed 1');
$t->get_ok('/')->status_is(200)->content_like(qr/Whoa/i, 'Test Routing Failed 2');

# test logout
$t->get_ok('/logout')->status_is(200)->content_like(qr/login/i, 'Test Logout');

# test that we really are logged out
$t->get_ok('/')->status_is(200)->content_like(qr/login/i, 'Test Login screen landing 4');
$t->get_ok('/api')->status_is(200)->content_like(qr/login/i, 'Test Login screen landing 5');
$t->get_ok('/admin')->status_is(200)->content_like(qr/login/i, 'Test Login screen landing 6');
...
$t->post_ok('/admin/users',
  json => {email => 'test@test.com', password => 'password', reset_password => 1, is_admin => 0,})->status_is(201)
...
$t->post_ok('/admin/users', json => {email => 'test@test.com', password => 'password', is_admin => 0,})->status_is(409);
...
$t->post_ok('/auth/login', form => {username => 'test@test.com', password => 'password'})->status_is(200)
  ->element_exists('[name=current-password]')->element_exists('[name=new-password]')
...
$t->post_ok('/auth/password/change',
  form => {'current-password' => 'blah', 'new-password' => 'blah2', 'retyped-new-password' => 'blah2'})->status_is(200)
...
$t->post_ok('/auth/password/change',
  ...
  ->status_is(200)->content_like(qr/Password Change Failed: New Password cannot equal the existing password/,
...
$t->post_ok('/auth/password/change',
  ...
  ->status_is(200)->content_like(qr/Password Change Failed: New Password does not equal Retyped New Password/,
...
$t->post_ok('/auth/password/change',
  ...
  ->status_is(200)->content_like(qr/Password Change Failed: New Password does not meet complexity requirements/,
...
$t->post_ok('/auth/password/change',
  ...
  ->status_is(200)->content_like(qr/Whoa/, 'Password Change OK - Re-directs to root route');

# check that non-admin's can't access the admin interface
$t->get_ok('/admin')->status_is(200)->content_like(qr/Whoa/, 'Non-Admins cant access the Admin interface');

# check that non-admin's can't access the admin API either
$t->get_ok('/users')->status_is(403, 'Non-admins cant access Admin API');

# log the user out
$t->get_ok('/logout')->status_is(200)->content_like(qr/Login/i);

# login as admin to expire the user's password via date
$t->get_ok('/admin')->status_is(200)->content_like(qr/Login/i);

$t->post_ok('/auth/login', form => {username => 'admin@test.com', password => 'testpass'})->status_is(200)
  ->content_like(qr/Admin/, 'Make sure we are not at the login page anymore');

$t->put_ok('/admin/users', json => {email => 'test@test.com', last_reset => '2022-01-01T00:00:00Z'})->status_is(200)
  ->json_is('/email'          => 'test@test.com')->json_is('/dod_id' => undef)->json_is('/is_admin' => 0)
  ->json_is('/reset_password' => 0)->json_is('/last_reset' => '2022-01-01T00:00:00Z')->json_hasnt('/password');

# logout the admin
$t->get_ok('/logout')->status_is(200)->content_like(qr/Login/i);

# login as the user - should get expired password / change screen
$t->post_ok('/auth/login', form => {username => 'test@test.com', password => 'blah234!'})->status_is(200)
  ->element_exists('[name=current-password]')->element_exists('[name=new-password]')
  ->element_exists('[name=retyped-new-password]')

...
subtest 'Test User Login/Logout/Admin operations' => sub {
  ...
  $t->get_ok('/admin')
    ->status_is(403, 'Non-Admins cant access the Admin interface');
...
subtest 'Test user account updates cannot modify read-only fields' => sub {
  ...
  $t->get_ok('/admin/users?email=admin@test.com')->status_is(200)->tap(sub ($t) {
  ...
  $t->get_ok('/admin/users?email=admin@test.com')->status_is(200)

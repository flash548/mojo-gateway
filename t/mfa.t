...
subtest 'Check cannot navigate to the MFA endpoints when being signed in' => sub {
  ...
  $t->get_ok('/auth/mfa/init')->content_like(qr/This is a restricted page/, 'Logged in - Cannot get to the MFA init page - 1');
  $t->post_ok('/auth/mfa/init')->content_like(qr/This is a restricted page/, 'Logged in - Cannot get to the MFA init page - 2');
  $t->get_ok('/auth/mfa/entry')->content_like(qr/This is a restricted page/, 'Logged in - Cannot get to the MFA entry page - 1');
  $t->post_ok('/auth/mfa/entry')->content_like(qr/This is a restricted page/, 'Logged in - Cannot get to the MFA entry page - 2');
};
...
subtest 'Test user forced to enroll into the MFA when all users forced' => sub {
  ...
  $t->post_ok('/auth/mfa/init')
    ->content_like(qr/Whoa/, 'Check done with MFA enrollment and back to requested page');
  ...
  my $code = $auth->code;
  $t->post_ok('/auth/mfa/entry' => form => { 'mfa-entry' => $code })
    ->content_unlike(qr/MFA/);
  ...
  ok !defined(($t->app->db_conn->db->select('users', undef, { email => 'admin@test.com'} )
    ->hashes
    ->[0]
    ->{mfa_secret}));

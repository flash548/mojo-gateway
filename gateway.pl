...
app->db_conn->db->insert(
  ...
  unless defined(
  app->db_conn->db->select(
    'users', undef, { email => $config->{admin_user} }
  )->hash
  );
...
sub proxy ( $c, $name, $uri ) {
  ...
  if ( $config->{routes}->{$name}->{enable_jwt} ) {
    ...
    for my $claim ( keys %{ $config->{routes}->{$name}->{jwt_claims} } ) {
      $claims->{$claim} =
        eval $config->{routes}->{$name}->{jwt_claims}->{$claim};
    }
    my $jwt = Mojo::JWT->new(
      claims => $claims,
  ...
  for my $header ( keys %{ $config->{routes}->{$name}->{other_headers} } ) {
    $request->headers->add( $header,
      $config->{routes}->{$name}->{other_headers}->{$header} );
  }
...
under '/' => sub ($c) {
  ...
  if (($c->session->{user}->{reset_password} || ( defined( $config->{password_valid_days} )&& $config->{password_valid_days} > 0 )) && $c->req->url !~ /auth\/password\/change/) {
    # because we said so
...
post '/admin/users' => sub ($c) {
  add_user($c);
};

get '/users' => sub ($c) {
  $c->render('no_response') if !check_user_admin($c);
  my $users = app->db_conn->db->select( 'users',[ 'email', 'reset_password', 'dod_id', 'last_reset', 'last_login', 'is_admin' ] )->hashes;
  $c->render( json => $users );
};

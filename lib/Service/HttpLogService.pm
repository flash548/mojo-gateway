...
sub end_trace ($self, $c) {
  ...
  return if ($self->config->{logging_ignore_paths} 
...
sub _validate_search_params($self,
  $c,
  $status_code_is, 
  $user_email_contains, 
  $path_contains, 
  $request_method_is,
  $user_agent_contains,
  $request_host_contains,
  $query_string_contains,
  $time_taken_ms_less_than,
  ...
  if (defined($request_method_is) && $request_method_is =~ m/;/) {
    $c->render(json => { message => "Request Method contains invalid characters" }, status => Constants::HTTP_BAD_REQUEST);
    return;
  }
...
sub get_logs ($self,
  ...
  return unless $self->_validate_search_params($c,$status_code_is, 
    $user_email_contains, 
    $path_contains, 
    $request_method_is,
    $user_agent_contains,
  ...
  my ($query, $query_bindings, $total_count_query, $total_count_bindings) = $self->_build_query($page, 
    $page_size, 
    $from_date, 
    $to_date, 
    $status_code_is, 
